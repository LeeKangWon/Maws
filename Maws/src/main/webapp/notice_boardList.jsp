...
<%
	...
	try {
		...
		// 게시물의 총 수를 얻는 쿼리 실행
		pstmt = conn.prepareStatement("SELECT COUNT(NUM) AS TOTAL FROM notice_board" + whereSQL);
		if (!"".equals(whereSQL)) {
			if ("ALL".equals(searchType)) {
		...
		// 게시물 목록을 얻는 쿼리 실행
		pstmt = conn.prepareStatement("SELECT num, sub, writer, reg_date, hit FROM notice_board "
	...
	<div style="position: absolute; left: 430px; top: 260px;">
		<table width="100%" cellpadding="0" cellspacing="0" border="0">
			...
			<td></td>
				...
				<td><a href="notice_boardView.jsp?num=<%=rs.getInt("num")%>&amp;pageNum=<%=pageNumTemp%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>"><%=rs.getString("sub")%></a></td>
						...
						<%
							// 페이지 네비게이터
								if (totalCount > 0) {
									...
									if (pageNumTemp > 1) {
										sb.append("<a href=\"").append("notice_boardList.jsp?pageNum=1&amp;searchType=" + searchType
												+ "&amp;searchText=" + searchText);
										sb.append("\" title=\"<<\"><<</a>&nbsp;");
									}
									if (isPrev) {
										int goPrevPage = startPage - pagePerBlock;
										sb.append("&nbsp;&nbsp;<a href=\"").append("notice_boardList.jsp?pageNum=" + goPrevPage
									...
									for (int i = startPage; i <= endPage; i++) {
										if (i == pageNumTemp) {
											...
											sb.append("<a href=\"").append("notice_boardList.jsp?pageNum=" + i + "&amp;searchType="
									...
									if (isNext) {
										...
										sb.append("<a href=\"").append("notice_boardList.jsp?pageNum=" + goNextPage + "&amp;searchType="
												+ searchType + "&amp;searchText=" + searchText);
										sb.append("\" title=\">\">></a>");
									} else {
									...
									if (totalNumOfPage > pageNumTemp) {
										sb.append("&nbsp;&nbsp;<a href=\"").append("notice_boardList.jsp?pageNum=" + totalNumOfPage
												+ "&amp;searchType=" + searchType + "&amp;searchText=" + searchText);
										sb.append("\" title=\">>\">>></a>");
									}
		...
		<p>
			<input align="right" type="button" value="목록" onclick="goUrl('notice_boardList.jsp');" />
			<input align="right" type="button" value="글쓰기"
				onclick="goUrl('notice_boardWriteForm.jsp');" />
		</p>
		<form name="searchForm" action="notice_boardList.jsp" method="get"

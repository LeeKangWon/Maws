...
<%
	...
	try {
		...
		// 조회수 증가 쿼리 실행
		pstmt = conn.prepareStatement("UPDATE conference_board SET hit = hit + 1 WHERE num = ?");
		pstmt.setString(1, num);
		pstmt.executeUpdate();
		// 게시물 상세 조회 쿼리 실행 
		pstmt = conn.prepareStatement(
				"SELECT num, sub, content, writer, reg_date, hit FROM conference_board " + "WHERE num = ?");
		pstmt.setString(1, num);
		...
		<br> <br> <br> <br>
						...
						<p class="btn_align">
							...
							<input type="button" value="수정"
								onclick="goUrl('conference_boardModifyForm.jsp?num=<%=num%>&amp;pageNum=<%=pageNum%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>');" />
							<input type="button" value="삭제"
								onclick="deleteCheck('conference_boardProcess.jsp?mode=D&amp;num=<%=num%>&amp;pageNum=<%=pageNum%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>');" />
						</p>
	</div>
</body>
